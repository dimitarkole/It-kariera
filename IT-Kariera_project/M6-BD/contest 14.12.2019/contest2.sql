...
Select 
	(
		Case 
			WHEN c.birth_date > '1980-01-01'
				Then concat(Year(birth_date), Day(birth_date) , Substring(c.ucn,0,4) )
			ELSE concat(Year(birth_date), Month(birth_date) , Substring(c.ucn,7,4) )
		End
	) As card_number,
    (
		Case 
    ...
    c.id,
    convert(substring(c.ucn,1,1), Signed int) as journey_id
    from colonists as c
